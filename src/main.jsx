...
function App() {
  ...
  const analysisColumns = useMemo(() => ([
    ...
    { id: 'rollEquivalent', label: '滚奏等效', sortable: true, renderCell: (item) => formatNumber(item.ratings.rollEquivalent) },
  ...
  function exportResults() {
    ...
    const rows = ['分类,歌曲,难度,星级,分支,Note数,体力,复合,复合难占比,节奏,节奏占比,手速,滚奏等效,爆发'];
    ...
    for (let songIndex = 0; songIndex < allResults.length; songIndex += 1) {
      ...
      for (const chart of song.charts) {
        ...
        rows.push(
          `"${song.category}","${song.songName}","${difficultyLabel}","${level || ''}","${branchLabel}",${chart.ratings.totalNotes || 0},${chart.ratings.stamina},${chart.ratings.complex},${chart.ratings.complexRatio},${chart.ratings.rhythm},${chart.ratings.rhythmRatio},${chart.ratings.speed},${chart.ratings.rollEquivalent},${chart.ratings.burst}`
        );
      }
    }
